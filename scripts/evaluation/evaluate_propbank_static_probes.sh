...
for (( i=0; i<$len; i++ )); do
    ...
    python evaluate_on_roleset_classification.py \
        --inventory propbank \
        --datamodule logs_probing/propbank_static/${MODEL}/config.json \
        --test_path data/json/${TEST_PATH} \
        --checkpoint logs_probing/propbank_static/${MODEL}/checkpoints/msrl*.ckpt \
        --output output/roleset_predictions/propbank_static/${MODEL}.txt
    printf "Done!\n\n"
done
