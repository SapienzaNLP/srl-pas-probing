...
def read_mapping(path):
    ...
    with open(path) as f:
        ...
        for line_index, line in enumerate(f):
            ...
            if '2009' in pb_sense:
                pb_sense = pb_sense[:-len('.2009')]
            elif pb_sense in predicate_mapping:
                continue
...
def remap(path, frame_names, predicate_mapping, role_mapping, use_default_mapping=False, include_RC_roles=False):
    ...
    for sentence_id, sentence in data.items():
        ...
        for predicate_index, (predicate, pos_tag) in enumerate(zip(predicates, pos_tags)):
            ...
            try:
                predicate_roles = roles[str(predicate_index)]
            except:
                print()
                print(sentence_id)
            ...
            for role in predicate_roles:
                ...
                if role in role_mapping[predicate]:
...
if __name__ == '__main__':
    ...
    remapped_data = remap(args.input_path, frame_names, predicate_mapping, role_mapping, use_default_mapping=True, include_RC_roles=True)
